...
%find the processor group with the highest computational capability - HCCP
...
% define the cpu_Ref (reference processor)
 cpu_ref=6;
 
 % When the Highest Computation Capability Processor (HCCP) is a coprocessor, an extra loop kernel is added to further reduce the number
 % of simulations (please see lines 17-24 in Algorithm.3 in submitted paper)
 % EXT=2 gives lines 1-16 in Algorithm.3 in submitted paper
 % EXT=1 gives lines 1-24 in Algorithm.3 in submitted paper
 if (makespan_quality==0) %makespan quality is not important
     if (fastest_i==6) 
         EXT=2;
     elseif (fastest_i>6) 
         EXT=1;
         range(fastest_i,1)=range(fastest_i,1)/4; % defince a very small unrechable lower bound for the HCCP
     else 
         fprintf('/n/n ERROR - PROCESSOR #6 IS ALWAYS THE REFERENCE PROCESSOR AND THEREFORE IT HAS TO EXIST /n/n');
     end
 else                   %makespan quality is important
     EXT=2;
 end
...
%the following code works when in the DAG every child node has a larger node label than its parent
...
gpu_speedup=5; % how many times the slowest coprocessor is faster than cpu_ref (using the 1 core only)
 ...
 speedup(2)=1.5; speedup(3)=2; speedup(4)=2.8; speedup(5)=3; speedup(6)=3.5; 
  ...
  %good_speedup(2)=1.5; good_speedup(3)=2.2; good_speedup(4)=3.0;  good_speedup(5)=3.6; good_speedup(6)=4.1; 
...
%APPROXIMATE EFT ON ALL Processors. 

  for i=1:diff_nodes        %for each processor group (different processors)
    for j=1:common_nodes      %for each processor inside the group (common processors)
      for k=1:max_cores 
        if (HW(i,j,k)~=0) % if this processor exists   
           ...
           if (i>6) % if the processor is coprocessor, then it is faster than the best multithread
           ...
           %-------------find min EFT value for each processor group
    ...
    if ( flagN == 1 )
       % fprintf('\n 1o %d',ind);
                        %nodes are faced as a single thread
                           ...
                           emulations(ind,:)=0;
                           for it1=diff_nodes:-1:2
                             if ( (min_single(it1,1) ~= 99999) )  
                                for it2=(it1-1):-1:1
                                   if ( min_single(it1,1) <= min_single(it2,1) )
                                       emulations(ind,it2)=-1;
                                   end
                                end
                             else
                            ...
                            if ( fastest_i > 6 )  %if the fastest node is gpu 
                               ...
                               for it1=1:fastest_i-1
                                  ...
                                  for it1=1:diff_nodes
                                   ...
                                   if (ex_ind2 > 6) % if the node is gpu of fpga
