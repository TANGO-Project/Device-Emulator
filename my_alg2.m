...
function  [output,emulations,makespan,slr,em] = my_alg2 (A,D,HW,cpu_ref, range, THRES, THRESHOLD)
  ...
  for i=1:diff_nodes %for each processor
    for j=1:common_nodes
      for k=1:max_cores 
       ...
       min_gpu=min_single(4,1)-ex_times(ind)/speedup(max_cores)+ex_times(ind)*range(cpu_ref+1,1);
       for it1=1:diff_nodes-cpu_ref
