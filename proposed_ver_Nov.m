...
function  [output,emulations,makespan,speed_up,em,util,less_em] = proposed_ver_Nov (A,D,HW, range, THRES, THRESHOLD, THR,THR2)


[tasks,diff_nodes,max_cores]=size(D);
[diff_nodes, common_nodes, max_cores]=size(HW);

%find the processor group with the highest computational capability
...
% define the cpu_Ref. 
% S0S -> IN THE SPECIAL CASE WHERE THE FASTEST PROCESSOR IS #6 AND PROC #5 HAS A DIFFERENT # OF CORES THAN PROCESSOR 6, IS NOT TAKEN INTO ACCOUNT HERE
 cpu_ref=6;
 if (fastest_i==6)   %fastest_i and cpu_ref must contain the maximum number of cores
     %find # of fastest_i cores
     %find # of fastest_i-1 cores
     %if they are equal cpu_ref=6; else cpu_ref=5
     for i=6:-1:1
         if (HW(6,1,i)==1)
             cores1=i;
             break;
    ...
    for j=1:common_nodes
      for k=1:max_cores 
        if (HW(i,j,k)~=0) % if this processor exists   
           ...
           if (i>6) % if the node is GPU, FPGA, then it is faster than the best multithread
