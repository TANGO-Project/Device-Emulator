% it is described by algorithm4.txt 
%like my_alg2.m but when only single cores are used, I compute the emulations again


%like my_alg1.m but I use a lower bound to the fastest node in order to
...
function  [output,emulations,makespan,slr,em, util,less_em] = my_alg2b (A,D,HW,cpu_ref, range, THRES, THRESHOLD)
 ...
 for it2=it1:(it1+THRESHOLD)
     if ( it2 <=tasks )
       if ( ready(sort_tasks(it2)) ==0 )
         ...
         if ( not_parallel==1 ) %take the min EFT solution no matter on how many cores 
