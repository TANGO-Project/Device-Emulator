%slightly lower quality solution than my1 because the new inequality has
%been added here uses ex_times() not D()

function  [output,emulations,makespan,slr,em] = my1_b (A,D,HW,cpu_ref,range)

THREASHOLD=range(cpu_ref+1,1);
  ...
  for i=1:diff_nodes %for each processor
    for j=1:common_nodes
      for k=1:max_cores 
       ...
       min_gpu=min_single(4,1)-ex_times(ind)/speedup(max_cores)+ex_times(ind)*THREASHOLD;
...
fprintf('\n Proposed_1b - makespan=%f, speedup=%f, efficiency=%f, SLR=%f',makespan,speed_up,speed_up/(total_num_cores_nodes),slr );
