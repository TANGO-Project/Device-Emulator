...
function  [output,emulations,makespan,speed_up,em,util,less_em] = my_alg1c_ver_Nov (A,D,HW,cpu_ref, THRES, THRESHOLD, THR,THR2)
             ...
             if (counter<=THR)
                ...
                for j=1:tasks-1
                    if ( A(j,cur_task) ~= 0 )  % for all the parents of cur_task
                        if ( (A(j,cur_task)/ex_times(cur_task)) >= 1 ) % if a high communication edge exists
                            high_com_tasks=high_com_tasks+1;
                            break;
                        end
                    end
                    if ( A(cur_task,j) ~= 0 )  % for all the children of cur_task
                        if ( (A(cur_task,j)/ex_times(cur_task)) >= 1 ) % if a high communication edge exists
                            high_com_tasks=high_com_tasks+1;
                            break;
                        end
                    end
